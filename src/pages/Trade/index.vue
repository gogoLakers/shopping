  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      >
        ...
        >
        <p @click="changeSelect(userAddress, userAddressList)">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ findDefault.fullAddress }}</span>
        收货人：<span>{{ findDefault.consignee }}</span>
        <span>{{ findDefault.phoneNum }}</span>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    // 请求用户地址(只能用老师的账号 id：13700000000    password：111111)
    this.$store.dispatch("userAddressList");
    // 请求订单数据
    this.$store.dispatch("tradeList");
  },
  computed: {
    ...mapGetters(["tradeList", "userAddressList"]),
    // 商品明细
    detailArrayList() {
      return this.tradeList.detailArrayList || [];
    },
    // find选出数组中符合条件的元素，返回的是元素(这里是一个对象)    filter返回的是一个数组
    findDefault() {
  ...
  methods: {
    // 改变默认地址
    changeSelect(userAddress, userAddressList) {
    ...
    async submitOrder() {
      ...
      let data = {
        consignee: this.findDefault.consignee,
        consigneeTel: this.findDefault.phoneNum,
        deliveryAddress: this.findDefault.fullAddress,
        paymentWay: "ONLINE",
        orderComment: this.text,
