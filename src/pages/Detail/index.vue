        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd
                  ...
                  :class="{ active: parseInt(spuSaleAttrValue.isChecked) }"
                  @click="
                    changeActive(
                      spuSaleAttrValue,
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a href="javascript:;" @click="sendAddToCart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    // 给放大镜传的数据需要考虑空数据的情况，防止undefined
    skuImageList() {
  ...
  methods: {
    changeActive(saleAttrValue, saleAttrValueList) {
      // 将同为一种属性的active类名都去除，再将点击的加上active类名（排他思想）
      saleAttrValueList.forEach((saleAttr) => (saleAttr.isChecked = "0"));
      saleAttrValue.isChecked = "1";
    },
    // 购买数量数字限制
    changeSkuNum() {
      let value = this.skuNum * 1; //字符串*1为NaN
      if (isNaN(value) || value < 1) {
        alert("您输入的商品数量格式错误,请重新输入");
        this.skuNum = 1;
      } else {
        this.skuNum = parseInt(value);
      }
    },
    // 点击加入购物车，跳转前发送请求是否发送成功的信息，成功则跳转路由，否则弹出错误信息
    async sendAddToCart() {
      // 向actions传递两个参数，，因为发送请求时要携带商品Id还有数量
      //dispatch会接收到actions里发请求返回的promise，然后根据状态进行相应操作
      try {
